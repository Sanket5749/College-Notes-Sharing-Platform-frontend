...
export const UploadModal = ({ isOpen, onClose, subjects, onUploaded }) => {
  ...
  const branchObj =
    getBranchInfo(selectedBranch) || ALLOWED_BRANCHES[0];
  ...
  const filteredSubjects = subjects.filter((s) => {
    // 1. Check semester
    if (semester && String(s.semester) !== String(semester)) {
      return false;
    }

    // 2. For Semesters 1 and 2,
    // include Common Engineering + Branch subjects
    if (parsedSemester <= 2) {
      return (
        ...
        (s.department &&
          s.department
            .toLowerCase()
            .includes(branchObj.code.toLowerCase()))
      );
    }

    // 3. Semesters 3-8:
    // strictly branch subjects
    return (
      s.department === targetBranchName ||
      (s.department &&
        s.department
  ...
  const resetForm = () => {
    ...
    setTitle('');
    setDescription('');
    setSelectedBranch(ALLOWED_BRANCHES[0].name);
    setSemester('1');
    setSubjectId('');
    setIsDragOver(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title.trim() || !semester || !subjectId) {
      showToast(
        'Please complete all required fields (Title, Branch, Semester, Subject).',
        'warning'
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        'Study note uploaded and published successfully!',
        'success'
      );
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={handleCancel}
          disabled={uploading}
          className="absolute top-5 right-5 inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-semibold text-slate-300 bg-slate-800 border border-slate-700 hover:bg-slate-700 hover:text-white transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          aria-label="Cancel"
        >
          <X size={16} />
        </button>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              <span className="text-sm font-semibold text-slate-200">
                {file
                  ? 'Replace selected PDF'
                  : 'Click to browse or drag & drop PDF'}
              </span>

              <span className="text-xs text-slate-500">
                Max file size: 10 MB (Strictly PDF)
              </span>
            </div>
          ...
          <button
            type="submit"
            className="w-full mt-2 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 shadow-lg shadow-indigo-500/25 active:scale-[0.98] transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={uploading}
          >
